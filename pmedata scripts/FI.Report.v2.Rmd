...
if (end.date != valdate %m+% months(3)){
...
if (index(od.pcaps[dim(od.pcaps)[1]]) != end.date) {
   od.pcaps <- rbind(od.pcaps,xts(matrix(0, ncol = dim(od.pcaps)[2]), order.by = as.Date(end.date)))
}
...
#   xts(.) %>%
#   multiply_by(-1) %>%
#   rbind(., xts(matrix(0, ncol = dim(od.pcaps)[2], nrow = dim(od.pcaps)[1]), order.by = as.Date(index(od.pcaps))))
...
od.irr.qtr = xts(matrix(0, ncol = dim(od.pcaps)[2], nrow = dim(od.pcaps)[1]), order.by = as.Date(index(od.pcaps)))
od.bench.qtr <- xts(matrix(0, ncol = 1, nrow = dim(od.pcaps)[1]), order.by = as.Date(index(od.pcaps)))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
date.range <- index(od.pcaps)
od.irr.year = xts(matrix(0, ncol = dim(od.pcaps)[2], nrow = dim(od.pcaps)[1]), order.by = as.Date(index(od.pcaps)))
od.bench.year <- xts(matrix(0, ncol = 1, nrow = dim(od.pcaps)[1]), order.by = as.Date(index(od.pcaps)))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
date.range <- index(od.pcaps)
od.irr.three = xts(matrix(0, ncol = dim(od.pcaps)[2], nrow = dim(od.pcaps)[1]), order.by = as.Date(index(od.pcaps)))
od.bench.three <- xts(matrix(0, ncol = 1, nrow = dim(od.pcaps)[1]), order.by = as.Date(index(od.pcaps)))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
date.range <- index(od.pcaps)
od.irr.five = xts(matrix(0, ncol = dim(od.pcaps)[2], nrow = dim(od.pcaps)[1]), order.by = as.Date(index(od.pcaps)))
od.bench.five <- xts(matrix(0, ncol = 1, nrow = dim(od.pcaps)[1]), order.by = as.Date(index(od.pcaps)))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
date.range <- index(od.pcaps)
od.irr.itd = xts(matrix(0, ncol = dim(od.pcaps)[2], nrow = dim(od.pcaps)[1]), order.by = as.Date(index(od.pcaps)))
od.bench.itd <- xts(matrix(0, ncol = 1, nrow = dim(od.pcaps)[1]), order.by = as.Date(index(od.pcaps)))
...
if (index(pd.pcaps[dim(pd.pcaps)[1]]) != end.date) {
   pd.pcaps <- rbind(pd.pcaps,xts(matrix(0, ncol = dim(pd.pcaps)[2]), order.by = as.Date(end.date)))
}
...
pd.cf <- do.call(merge, cf.all) %>%
   ...
   xts(.) %>%
   multiply_by(-1) %>%
   rbind(., xts(matrix(0, ncol = dim(pd.pcaps)[2], nrow = dim(pd.pcaps)[1]), order.by = as.Date(index(pd.pcaps))))
...
pd.irr.qtr = xts(matrix(0, ncol = dim(pd.pcaps)[2], nrow = dim(pd.pcaps)[1]), order.by = as.Date(index(pd.pcaps)))
pd.bench.qtr <- xts(matrix(0, ncol = 1, nrow = dim(pd.pcaps)[1]), order.by = as.Date(index(pd.pcaps)))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
date.range <- index(pd.pcaps)
pd.irr.year = xts(matrix(0, ncol = dim(pd.pcaps)[2], nrow = dim(pd.pcaps)[1]), order.by = as.Date(index(pd.pcaps)))
pd.bench.year <- xts(matrix(0, ncol = 1, nrow = dim(pd.pcaps)[1]), order.by = as.Date(index(pd.pcaps)))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
date.range <- index(pd.pcaps)
pd.irr.three = xts(matrix(0, ncol = dim(pd.pcaps)[2], nrow = dim(pd.pcaps)[1]), order.by = as.Date(index(pd.pcaps)))
pd.bench.three <- xts(matrix(0, ncol = 1, nrow = dim(pd.pcaps)[1]), order.by = as.Date(index(pd.pcaps)))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
date.range <- index(pd.pcaps)
pd.irr.five = xts(matrix(0, ncol = dim(pd.pcaps)[2], nrow = dim(pd.pcaps)[1]), order.by = as.Date(index(pd.pcaps)))
pd.bench.five <- xts(matrix(0, ncol = 1, nrow = dim(pd.pcaps)[1]), order.by = as.Date(index(pd.pcaps)))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
date.range <- index(pd.pcaps)
pd.irr.itd = xts(matrix(0, ncol = dim(pd.pcaps)[2], nrow = dim(pd.pcaps)[1]), order.by = as.Date(index(pd.pcaps)))
pd.bench.itd <- xts(matrix(0, ncol = 1, nrow = dim(pd.pcaps)[1]), order.by = as.Date(index(pd.pcaps)))
