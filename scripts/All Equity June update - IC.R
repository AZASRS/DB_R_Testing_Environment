...
totoppreturn.df = data.frame("Category" = rownames(totoppreturn),
                          totoppreturn, row.names = NULL)
colnames(totoppreturn.df)[-1] <- colnames(totoppreturn)
totoppreturn.df$Qtr = paste0(round(totoppreturn.df$Qtr*100,1),"%")
totoppreturn.df$`1 Year` = paste0(round(totoppreturn.df$`1 Year`*100,1),"%")
totoppreturn.df$`3 Year` = paste0(round(totoppreturn.df$`3 Year`*100,1),"%")
totoppreturn.df$`5 Year` = paste0(round(totoppreturn.df$`5 Year`*100,1),"%")
totoppreturn.df$ITD = paste0(round(totoppreturn.df$ITD*100,1),"%")
hlines = c(-1, 0, 2, dim(totoppreturn.df)[1])
print(
  xtable(totoppreturn.df, digits = 2), 
